...
bool TellLosAction::Execute(Event& event)
{
    ...
    {
        ListUnits(requester, "--- 目标 ---", AI_VALUE(std::list<ObjectGuid>, "possible targets"));
        ListUnits(requester, "--- 目标 (全部) ---", AI_VALUE(std::list<ObjectGuid>, "all targets"));
    }
    ...
    {
        ListUnits(requester, "--- 尸体 ---", AI_VALUE(std::list<ObjectGuid>, "nearest corpses"));
    }
    ...
    if (param.empty() || param.find(GOS_PARAM) == 0 || param.find(GAMEOBJECTS_PARAM) == 0)
    {
        ...
        TellGameObjects(requester, "--- 物体 ---", FilterGameObjects(requester, GoGuidListToObjList(ai, AI_VALUE(std::list<ObjectGuid>, "nearest game objects no los")), mods), mods);
    }

    if (param.empty() || param == "players")
    {
        ListUnits(requester, "--- 友善玩家 ---", AI_VALUE(std::list<ObjectGuid>, "nearest friendly players"));
    }
