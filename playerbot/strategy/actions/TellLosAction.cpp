...
bool TellLosAction::Execute(Event& event)
{
    ...
    {
        ListUnits(requester, "--- 目标 ---", *context->GetValue<list<ObjectGuid> >("possible targets"));
        ListUnits(requester, "--- 目标 (全部) ---", *context->GetValue<list<ObjectGuid> >("all targets"));
    }
    ...
    {
        ListUnits(requester, "--- 尸体 ---", *context->GetValue<list<ObjectGuid> >("nearest corpses"));
    }

    if (param.empty() || param == "gos" || param == "game objects")
    {
        ListGameObjects(requester, "--- 物体 ---", *context->GetValue<list<ObjectGuid> >("nearest game objects"));
    }

    if (param.empty() || param == "players")
    {
        ListUnits(requester, "--- 友善玩家 ---", *context->GetValue<list<ObjectGuid> >("nearest friendly players"));
    }
