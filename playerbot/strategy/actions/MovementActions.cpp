...
bool MovementAction::MoveTo(uint32 mapId, float x, float y, float z, bool idle, bool react, bool noPath, bool ignoreEnemyTargets)
{
    ...
    if (!movePath.empty())
    {
        ...
        if (!bot->GetTransport() && movePath.makeShortCut(startPosition, sPlayerbotAIConfig.reactDistance, bot))
            if (ai->HasStrategy("debug move", BotState::BOT_STATE_NON_COMBAT))
                ai->TellPlayerNoFacing(GetMaster(), "发现捷径:原距离=" + std::to_string(uint32(oldDist)) + "码,新距离=" + std::to_string(uint32(WorldPosition().getPathLength(movePath.getPointPath()))));
        ...
        if (pathType == TravelNodePathType::transport)
        {
            if (!bot->GetTransport()) //We are not yet on a transport.
            {
                for (auto& transport : startPosition.getTransports(entry))
                {
                    ...
                    {
                        if (ai->HasStrategy("debug move", BotState::BOT_STATE_NON_COMBAT))
                            ai->TellPlayer(GetMaster(), "在入口 " + std::to_string(uint32(telePosition.distance(transport))) + "码进行传送");

                        if (telePosition.distance(transport) < INTERACTION_DISTANCE) //Transport has arrived Move on.
                        {
                            if (ai->HasStrategy("debug move", BotState::BOT_STATE_NON_COMBAT))
                                ai->TellPlayerNoFacing(GetMaster(), "前往传送 " + std::string(transport->GetName()));
            ...
            else //We are on transport.
            {
                ...
                {
                    if (ai->HasStrategy("debug move", BotState::BOT_STATE_NON_COMBAT))
                        ai->TellPlayerNoFacing(GetMaster(), "我在 " + std::string(bot->GetTransport()->GetName()));
                    ai->SetMoveToTransport(false);
                    entry = 0;
                }

                if (movePosition.getMapId() == bot->GetMapId() && ai->HasStrategy("debug move", BotState::BOT_STATE_NON_COMBAT))
                    ai->TellPlayer(GetMaster(), "传送在 " + std::to_string(uint32(telePosition.distance(bot->GetTransport()))) + "yards of exit");
...
bool MovementAction::Flee(Unit *target)
{
    ...
    if (!IsMovingAllowed())
    {
        ai->TellError(GetMaster(), "我在逃跑时被困住了");
        return false;
    }
    ...
    if(!succeeded)
    {
        ...
        {
            ai->TellError(GetMaster(), "没有地方可逃跑");
            return false;
        }
...
bool MoveToLootAction::Execute(Event& event)
{
    ...
    {
        std::ostringstream out;
        out << "走过去拾取 " << ChatHelper::formatWorldobject(wo);
