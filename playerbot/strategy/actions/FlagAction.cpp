...
bool FlagAction::Execute(Event& event)
{
    ...
    if (ss[0] == "pvp")
    {
        ...
        std::ostringstream out; out << ss[0] << " flag是 " << chat->formatBoolean(bot->IsPvP());
    ...
    std::ostringstream out; out << ss[0] << " flag是 " << chat->formatBoolean(!bot->HasFlag(PLAYER_FLAGS, playerFlags));
    ai->TellPlayer(requester, out.str());
    return true;
}
