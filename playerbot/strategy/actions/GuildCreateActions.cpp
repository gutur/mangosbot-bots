...
bool PetitionOfferNearbyAction::Execute(Event& event)
{
    ...
    for (auto& i : nearGuids)
    {
        ...
        if (sPlayerbotAIConfig.inviteChat && sServerFacade.GetDistance2d(bot, player) < sPlayerbotAIConfig.spellDistance && sRandomPlayerbotMgr.IsFreeBot(bot))
        {
            ...
            if(urand(0,3))
                bot->Say(BOT_TEXT2("嘿, %name ,你想一起创建一个公会吗?", placeholders), (bot->GetTeam() == ALLIANCE ? LANG_COMMON : LANG_ORCISH));
            else
                bot->Say(BOT_TEXT2("嘿, 你想组建一个公会吗?", placeholders), (bot->GetTeam() == ALLIANCE ? LANG_COMMON : LANG_ORCISH));
        }
